...
### Đề án cải tiến (`proposalRecords`) + Nội dung chờ duyệt (`contentSubmissionRecords`) — cập nhật 17/09/2026 ("Tinh chỉnh Cổng quản lý")
- **`proposalSeedRecords`** (3 bản ghi PRP-001/PRP-002/PRP-003, `data/pilot-seed-data.js`) — đề án/góp ý do Host gửi lên Cổng quản lý, seed 1 lần vào `state.proposals` khi khởi tạo (idempotent backfill theo `id`, không seed lại đè lên bản ghi Host/Management đã xử lý). `providerId` được ánh xạ từ `activityCatalog[activityId].providerAccountId` (không dùng ID hư cấu `PROVIDER-01/02/03` như trong yêu cầu gốc, vì các ID đó không tồn tại trong dữ liệu host thật của app) — đảm bảo mỗi đề án hiển thị đúng ở trang Host tương ứng.
- **Điểm đọc DUY NHẤT**: `state.proposals`, dùng chung bởi Studio (`js/studio/support.js`, Host tự xem/gửi đề án) và Cổng quản lý (`js/admin/proposals.js`, xử lý Duyệt/Yêu cầu bổ sung/Từ chối) — không có bản sao riêng. Đổi trạng thái ở Cổng quản lý gọi `setProposalStatus()` trong `js/storage.js`, tự bắn thông báo cho đúng Host qua `addProviderNotification()` (kênh riêng `state.providerNotifications`, tách khỏi thông báo Khách) trừ khi chính Host là người vừa đổi trạng thái đó (`PROPOSAL_HOST_INITIATED_STATUSES`).
- Trường mới (`summary`/`requestedChange`/`expectedImpact`/`managementNote`) tồn tại song song trường cũ (`problem`/`desiredSupport`/`expectedBenefit`) — mọi nơi hiển thị dùng `newField || oldField`, không phá đề án cũ đã có trước phase này.
- **`contentSubmissionSeedRecords`** (2 bản ghi SUB-EXP-001/SUB-EXP-002) — mô phỏng hoạt động mới Host đã gửi qua Studio ("Thêm trải nghiệm") nhưng CHƯA được duyệt công bố. Nạp vào `state.contentSubmissions` cùng cơ chế idempotent backfill như trên.
- **Điểm đọc DUY NHẤT**: `state.contentSubmissions`, hiển thị ở cả Studio (lịch sử gửi của Host, chỉ xem) và Cổng vận hành (`js/ops/content.js`, xử lý Duyệt/Yêu cầu chỉnh sửa/Từ chối). Hoạt động **không xuất hiện** ở Activity Catalog / trang Khám phá của Khách cho tới khi được Duyệt — lúc đó `approveContentSubmission()` mới tạo bản ghi `destination` + `activityCatalog` mới (`publicationStatus:'published'`), đồng thời cập nhật cả mảng LIVE (`state.destinations`/`state.activityCatalog` qua `state.customActivityCatalog`) lẫn mảng PERSISTED (`state.customDestinations`) để hiện ngay không cần tải lại trang.
- Hoạt động được duyệt từ `contentSubmissions` mượn toạ độ/địa chỉ/ảnh từ **điểm đến gốc** của Host gửi (vì bản ghi submission không tự có các trường này) — không tạo pin bản đồ trống hoặc sai vị trí.
- **`hourlyDemandToday`** (`js/admin/flow.js`) — 6 khung giờ cố định dùng riêng cho biểu đồ cột "Khung giờ cao điểm dự kiến hôm nay", không gắn với booking cụ thể nào (khác với `weeklyDemandPattern` đã có từ trước) — nếu sau này có đủ booking thật trong ngày để tính `popularity` động, có thể thay bằng công thức, xem comment đầu file `flow.js`.
