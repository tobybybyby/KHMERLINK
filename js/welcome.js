...
export function renderWelcome(container) {
  container.innerHTML = `
    <div class="welcome-page welcome-page--hero">
      ${languageSwitcherHtml('language-toggle--floating')}
      <div>
        <div class="welcome-top-mark">${SIDE_MARK_LEFT}${LOTUS_ICON}${SIDE_MARK_RIGHT}</div>
        <h1 class="welcome-title">KhmerLink</h1>
        <p class="welcome-subtitle">${t('common.home.tagline')}</p>
        ${DIAMOND_DIVIDER}
        <p class="welcome-tagline" style="margin:var(--space-3) auto 0;">${t('common.home.heroText')}</p>
      </div>
      <nav class="welcome-choices" style="margin:0 auto;" aria-label="${t('common.a11y.roleChoice')}">
        <a class="welcome-choice" href="#/trail/explore">
          ...
          <span class="welcome-choice__body">
            <strong>${t('common.home.travellerTitle')}</strong>
            <span class="text-sm">${t('common.home.travellerDesc')}</span>
          </span>
        ...
        <a class="welcome-choice" href="#/studio">
          ...
          <span class="welcome-choice__body">
            <strong>${t('common.home.providerTitle')}</strong>
            <span class="text-sm">${t('common.home.providerDesc')}</span>
          </span>
          <span class="welcome-choice__arrow">${CHEVRON_ICON}</span>
        </a>
      </nav>
      <div>
        <a class="welcome-gateway-link" href="#/gateway">${t('common.brand.managementGateway')} →</a>
        ${DIAMOND_DIVIDER}
      </div>
    </div>
  `;
...
export function renderGateway(container) {
  container.innerHTML = `
    <div class="welcome-page">
      ${languageSwitcherHtml('language-toggle--floating')}
      <div>
        <h1 style="font-size:1.5rem;">${t('common.gateway.title')}</h1>
        <p style="opacity:0.85;">${t('common.gateway.subtitle')}</p>
      </div>
      <div class="gateway-list">
        <a class="gateway-item" href="#/admin">
          <strong>${t('common.gateway.dataTitle')}</strong>
          <span class="text-sm text-muted">${t('common.gateway.dataDesc')}</span>
        </a>
        <a class="gateway-item" href="#/ops">
          <strong>${t('common.gateway.opsTitle')}</strong>
          <span class="text-sm text-muted">${t('common.gateway.opsDesc')}</span>
        </a>
        <a class="gateway-item" href="#/ops/community">
          <strong>${t('common.gateway.communityTitle')}</strong>
          <span class="text-sm text-muted">${t('common.gateway.communityDesc')}</span>
        </a>
      </div>
      <a class="welcome-gateway-link" href="#/">${t('common.nav.backToHome')}</a>
    </div>
  `;
  wireLanguageSwitchers(container);
}
